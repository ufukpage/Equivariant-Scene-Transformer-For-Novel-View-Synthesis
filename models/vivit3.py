...
from models.module import PreNorm, FeedForward
...
class FSATransformerEncoder(nn.Module):
    ...
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, nt, nh, nw, dropout=0.):
        super(FSATransformerEncoder, self).__init__()
...
class FDATransformerEncoder(nn.Module):
    ...
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, nt, nh, nw, dropout=0.):
        super(FDATransformerEncoder, self).__init__()
        self.layers = nn.ModuleList([])
        self.nt = nt
        self.nh = nh
        self.nw = nw
...
class DeViT3(nn.Module):
    """ Model-3 backbone of ViViT """

    def __init__(self, d, h, w, patch_d, patch_h, patch_w, dim, depth, heads, mlp_dim, dim_head=3,
                 channels=3, mode='tubelet', device='cuda', emb_dropout=0., dropout=0., model=3):
        super(DeViT3, self).__init__()

        assert d % patch_d == 0 and h % patch_h == 0 and w % patch_w == 0, "Volume dimensions should be divisible by " \
                                                                           "tubelet size "
        ...
        self.nd = self.D // self.d
        self.nh = self.H // self.h
        self.nw = self.W // self.w

        tubelet_dim = self.d * self.h * self.w * channels
        ...
        # repeat same spatial position encoding depth-wise
        self.pos_embedding = nn.Parameter(torch.randn(1, 1, self.nh * self.nw, dim)).repeat(1, self.nd, 1, 1)

        self.dropout = nn.Dropout(emb_dropout)

        if model == 3:
            self.transformer = FSATransformerEncoder(dim, depth, heads, dim_head, mlp_dim,
                                                     self.nd, self.nh, self.nw, dropout)
        elif model == 4:
            assert heads % 2 == 0, "Number of heads should be even"
            self.transformer = FDATransformerEncoder(dim, depth, heads, dim_head, mlp_dim,
                                                     self.nd, self.nh, self.nw, dropout)
...
if __name__ == '__main__':
    ...
    vivit = ViViT3(32, 64, 64, 8, 4, 4, 10, 512, 6, 10, 8, model=3).to(device)
    out = vivit(x)
    print(out)
